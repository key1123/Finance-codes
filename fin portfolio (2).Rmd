...
AIG_ProfitLoss_date <- AIG_stock[300, ]

ylim = c(min(q_revenue_aig$Quarterly.Revenue), max(q_revenue_aig$Quarterly.Revenue))
plot(q_revenue_aig$Date, q_revenue_aig$Quarterly.Revenue, type='o', ylim=ylim, col="blue", xlab='Date', ylab='Revenue', lwd=2,frame.plot = FALSE)
...
abline(v = AIG_ProfitLoss_date, col = "red")
...
From the sales graph we can kinda see that the revenue for AIG since 2010 has been slowly decreasing until the year of the pandemic.
There is a trend with higher revenue leading to a new CEO but falling right after
there is an extreme peak or dip during 2013-2016 and again 2020-2021
...
From the Net Income Graph you can tell that overall the income for AIG stays between -5000 and 5000 except for 2012, 2018, and 2020 There is not a visible trend but when they hire a new CEO its usually after some time of a dip in Net Income. 
...
I can tell that I could have made 3 times my money had I invested during the pandemic. We can also see that AIG was hit hard by the pandemic based on their stock prices. The price of the stock stayed pretty steady until the 2020 pandemic. It seems as though the pandemic has put the company back on the right course, maybe even better than before. The company experienced several kinds of losses in the first couple of months after the pandemic. They have since recovered. They did not offer any incentives but AIG is an insurance company. More people could be afraid of the virus so they are taking out different policies for their families.  



c. Explain your company’s risk and return
▪ Find the monthly arithmetic return and standard deviation
▪ Find the geometric return
▪ Find the annual return and standard deviation
o What do these stats say about your company?
o Is there any news publication on your company that may explain performance?
...
library(quantmod)
library(psych)
AIG_stock["return"] <- Delt(AIG_stock$High)*100



ylim = c(-30, 30)
plot(AIG_stock$Date, AIG_stock$return, type='l', ylim=ylim, col="blue", xlab='Date', ylab='Price ', lwd=2,frame.plot = FALSE)
title("AIG Stock Return Percentage")
abline(v = pandemic_date, col = "red")
abline(v = Duperreault_date, col = "red")
abline(v = Zaffino_pres_date, col = "red")
abline(v = Zaffino_ceo_date, col = "red")
...
There is no defiant trend in the return percentages but AIG typically stays within a certain range, besides during the first year of the pandemic. Their returns usually dip after losses within the company. You can see a dip in 2018 after a major loss in profits for the year, as well as in 2020 when the pandemic hit. You can also see that during the first year of the pandemic the company had some of its highest percent returns. 


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}

AIG_stock["year"] <- as.numeric(format(AIG_stock$Date, "%Y"))
AIG_stock["month"] <- as.numeric(format(AIG_stock$Date, "%m"))
AIG_stock
monthly_returns <- AIG_stock %>% group_by(year, month) %>% summarize(return = mean(return))
monthly_returns
mean_monthly_returns <- mean(monthly_returns$return)
mean_monthly_returns
print("The monthly arithmetic return: ")
print("The standard deviation: ")
print("The annual return is : ")
print("The standard deviation of the annual return is : " )
```
